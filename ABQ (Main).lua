    ...
    if #lst_sword:GetChildren() > 2 then
        ...
        for swordname ,levelSword in pairs(swordDict) do
            ...
            if swordRarity == Rarity then
                table.insert(MergeRarityWeapon,lst_sword:FindFirstChild(swordname))
            end
        end
        ...
        while #MergeRarityWeapon >= 5 do
            ...
            RS.RF:InvokeServer("Merge",{(MergeRarityWeapon[1]),(MergeRarityWeapon[2]),(MergeRarityWeapon[3]),(MergeRarityWeapon[4]),(MergeRarityWeapon[5])},RarityNum[Rarity],"Swords")
        end
    end
end
...
local AutoSpecificDungeonTab = Window:CreateTab("Dungeon Create", nil)
...
local AutoCreateDugeonSpecificToggle = AutoSpecificDungeonTab:CreateToggle({
    ...
    Callback = function(Create)
        ...
        SaveSettingsToFile(settings)
        while settings.AutoCreateSpecMap do
            if game.PlaceId == 6494523288 and settings.map and settings.difficulty then            
                wait(2)
